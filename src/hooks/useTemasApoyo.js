...
const API_URL = "http://72.61.0.205:8000/api/academics/support-topics/";

export default function useTemasApoyo() {
    ...
    const createTema = async (data) => {
        ...
        delete cleanData.support_topic_id;
    ...
    const updateTema = async (id, data) => {
        await toast.promise(
            ...
            setTemas((prev) =>
                prev.map((t) => (t.support_topic_id === id ? res.data : t))
            );
        });
    };
    ...
    const deleteTema = async (id) => {
        await toast.promise(
            ...
            setTemas((prev) => prev.filter((t) => t.support_topic_id !== id));
        });
    };
