...
export default function RegistroPrediccionesModelo() {
        ...
        axios
            ...
            .then((res) => {
                ...
                const labeled = rawData.map((row) => {
                    ...
                    return { ...row, etiqueta_desempeno: etiqueta };
                });

                // Filtrar solo bajo y medio desempeño
                const filtered = labeled.filter((row) =>
                    ["Bajo", "Medio"].includes(row.etiqueta_desempeno)
                );

                setData(filtered);
                console.log("Predicciones filtradas:", filtered);
                setLoading(false);
            })
            .catch(() => {
    ...
    return (
        <CrudTable
            ...
            searchKeys={["output_model_prediction_id"]} 
        />
